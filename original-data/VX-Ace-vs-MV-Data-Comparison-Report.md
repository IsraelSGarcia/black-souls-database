# Data Comparison Report: VX Ace JSON-Converted vs MV-Converted

**Report Date:** Generated from systematic comparison  
**Data Sources:**
- VX Ace: `original-data/vx-ace-data/json-converted/`
- MV: `original-data/mv-converted/`

---

## Executive Summary

This report compares VX Ace JSON-converted data (direct conversion from `.rvdata2` Ruby Marshal files) with MV-converted data (converted to RPG Maker MV format). The analysis reveals both **structural/formatting differences** and **actual data value differences**.
...
**Impact:** MV has an additional trait with `code: 13` (TP Regeneration). However, according to `process-data.js`, TP has been removed from the database. This suggests the MV data may have been converted before TP removal, or this is a conversion artifact.

**Status:** ⚠️ **Data Difference** - Additional trait in MV (likely conversion artifact or pre-TP-removal data)

**Note:** Code 13 is TP-related and should be removed according to project documentation. This is a conversion artifact that should be cleaned from MV data.
...
**Analysis:** VX Ace uses a complex Tone object with multiple color channels, while MV uses a simplified 4-element array. The values are different, suggesting this may be a format conversion or the MV data was set to different default values.
...
**Issue:** MV data contains a TP-related trait (code 13) that should not be present according to project documentation.
...
**Recommendation:** Remove TP-related traits from MV data to match project documentation and VX Ace source.
...
1. **Clean MV Data**
   ...
   - Match VX Ace data structure

2. **Handle Weapon etypeId Differences**
   - Check if `etypeId: 1` in MV vs `etypeId: 0` in VX Ace affects empty weapons
   - Verify if this is intentional or conversion artifact
   - Consider normalizing to VX Ace values for accuracy
...
4. **Handle Missing Fields**
   ...
   - Don't assume MV fields exist in VX Ace
   - Don't assume VX Ace fields exist in MV
   - Use VX Ace field structure for VX Ace game analysis
...
3. **Use VX Ace for Parameter Analysis**
   - VX Ace has 8 standard parameters (0-7)
   - MV has 10 standard parameters (adds Hit, Evasion)
   - For VX Ace game analysis, use VX Ace parameter definitions
   - Hit and Evasion are Extended Parameters (8-9) in VX Ace, not standard parameters
...
- **Parameter definitions**: MV adds 2 parameters as standard (format difference)
- **Weapon defaults**: etypeId differs for empty weapons (conversion artifact)
- **Weapon 474**: Extra TP trait in MV (conversion artifact - should be removed)
...
**VX Ace JSON-converted data is the authoritative source** for this VX Ace game. MV-converted data is functionally equivalent for core game data but has some conversion artifacts (TP traits, etypeId defaults) and format differences. 

**Recommendation:** 
- Use **VX Ace data for accuracy** and analysis
- Use **MV data for convenience** (with verification and cleaning)
- **Remove TP traits** from MV data to match source
- **Verify Weapon etypeId** differences don't affect gameplay
- **Use VX Ace parameter definitions** for VX Ace game analysis
...
**Next Steps:** Clean MV data, verify Weapon etypeId differences, remove TP traits
